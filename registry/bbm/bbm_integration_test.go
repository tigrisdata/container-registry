	...
	return func() {
		base := []string{
			...
			fmt.Sprintf(
				...
				%s bigint,
				%s smallint)`, targetBBMTable, targetBBMColumn, targetBBMNewColumn, targetBBMNullColumn),
...
func BackfillNewIDWhereNull(ctx context.Context, db datastore.Handler, _, _ string, _, _, limit int) error {
	q := fmt.Sprintf(`
	...
	`, targetBBMTable, targetBBMNullColumn, targetBBMTable, targetBBMNullColumn, targetBBMColumn)
